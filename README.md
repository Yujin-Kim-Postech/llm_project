...
- `papers.jsonl`  
  **Raw data source.**  
  Contains the original paper metadata (one JSON object per line).  
  Each paper must include L1 and L2 category fields.

- `build_tree.py`  
  Script that reads `papers.jsonl` and constructs hierarchical tree data.

- `tree.json`  
  Tree-structured data generated by `build_tree.py`.  
